...
<style type="text/css">
    ...
    /* array */
    table.dBug_array { background-color:#6999AE;width:100%; }
    table.dBug_array td { background-color:#FFFFFF;}
    table.dBug_array td.dBug_arrayHeader { background-color:#6999AE; }
    table.dBug_array td.dBug_arrayKey { background-color:#BCC8DB;width:80px; }
    
    /* object */
    table.dBug_object { background-color:#EFAA42;width:100%; }
    table.dBug_object td { background-color:#FFFFFF; }
    table.dBug_object td.dBug_objectHeader { background-color:#EFAA42; }
    table.dBug_object td.dBug_objectKey { background-color:#F8DC85;width:80px; }
    ...
    function checkType($var) {
        switch(gettype($var)) {
            ...
            case 'boolean':
                ...
                break;
            default:
                $var = is_numeric($var) ? $var : ($var=='') ? '[empty string]' : $var;
                $var = dBug::$no_escape ? $var : htmlspecialchars($var);
        ...
        if(is_array($var)) {
            foreach($var as $key=>$value) {
                ...
                if(in_array(gettype($value),$this->arrType))
                    ...
                    $value = dBug::$no_escape ? $value : htmlspecialchars($value);
                    echo $value . "</td>\n</tr>\n";
                }
            }
        }
    ...
    function varIsObject($var) {
        ...
        if(is_object($var)) {
            foreach($arrObjVars as $key=>$value) {
                ...
                $value = (is_string($value) && $value=='') ? '[empty string]' : $value;
                $value = (is_array($value) && !$value) ? '[empty array]' : $value;
                $this->makeTDHeader("object",$key);
                if(in_array(gettype($value),$this->arrType)){
                    $this->checkType($value);
                } else{
                    $value = dBug::$no_escape ? $value : htmlspecialchars($value);
                    echo $value.$this->closeTDRow();
                }
            }
